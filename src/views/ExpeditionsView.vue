...
const creatureTierOptions = computed(() => {
  const tiers = new Set(
    recommendedCreatures.value.map(({ creature }) => creature.tier),
  );
  return Array.from(tiers).sort((a, b) => a - b);
});

const allCreatureTypesSelected = computed(() => {
  return creatureTypes.every((type) =>
    selectedCreatureTypes.value.includes(type),
  );
});
...
const allCreatureTiersSelected = computed(() => {
  return (
    ...
    creatureTierOptions.value.every((tier) =>
      selectedCreatureTiers.value.includes(tier),
    )
  );
});
...
watch(
  creatureTierOptions,
  (tiers) => {
    const preserved = selectedCreatureTiers.value.filter((tier) =>
      tiers.includes(tier),
    );
...
const filteredRecommended = computed(() => {
  return recommendedCreatures.value.filter(({ creature }) => {
    ...
    const matchesType = selectedCreatureTypes.value.some((type) =>
...
function selectExpedition(
  expedition: (typeof filteredExpeditions.value)[number],
) {
...
function chooseCreature(creature: Creature) {
  ...
  if (assignedCreatureIds.value.has(creature.id)) return; // safety
...
const assignedCreatureIds = computed(() => {
  return new Set(
    partySlots.value.filter((c): c is Creature => c !== null).map((c) => c.id),
  );
});

const displayRecommended = computed(() => {
  let list = filteredRecommended.value;

  if (ownedOnly.value) {
    list = list.filter(({ creature }) => isOwned(creature.id));
  }

  // 🔥 remove already assigned creatures
  return list.filter(
    ({ creature }) => !assignedCreatureIds.value.has(creature.id),
  );
});
      ...
      >
        ...
        >
          <h2 class="text-base font-bold">
            Expeditions ({{ totalXpPerSecond }}xp/s)
          </h2>
          <button class="pill focus-ring" @click="openSetupModal">
            Import / Export Setup
          </button>
        </div>

        <div
          class="max-h-[62vh] overflow-y-auto lg:min-h-0 lg:max-h-none lg:flex-1"
        >
          <button
          ...
          >
            ...
            >
              ...
              <span
                class="trait-chip"
                :class="expedition.trait ? '' : 'trait-chip-muted'"
              >
        ...
        >
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-2">
              ...
              <div
                class="inline-flex rounded-lg border border-border bg-muted/45 p-1"
              >
                <button
                  v-for="t in 5"
                  :key="t"
          ...
          <div class="grid grid-cols-3 gap-2 text-sm">
            ...
            <div class="rounded-lg border border-border bg-muted/35 px-3 py-2">
              ...
              <p class="font-semibold">
                {{ toTitleCase(selectedExpedition.biome) }}
              </p>
            </div>
            <div class="rounded-lg border border-border bg-muted/35 px-3 py-2">
              <p
              ...
              <p class="font-semibold text-amber-300">
                {{
                  selectedExpedition.trait
                    ? toTitleCase(selectedExpedition.trait)
                    : "None"
                }}
              </p>
            </div>
          ...
          <div class="grid grid-cols-2 gap-2 text-sm">
            ...
            <div class="rounded-lg border border-border bg-muted/35 px-3 py-2">
              ...
              <p class="font-semibold">
                {{ selectedExpedition.requiredExpeditionCompletions }}
              </p>
            </div>
          </div>
            ...
            <div class="flex flex-wrap gap-2">
              ...
              >
                <template v-if="slot">
                  ...
                  >
                    <p
                      class="min-w-0 truncate text-[10px] font-semibold text-white"
                    >
                      {{ slot.name }}
                    </p>
                    <span
                      class="shrink-0 font-mono text-[10px] font-semibold text-primary"
          ...
          >
            ...
            <div class="grid grid-cols-3 gap-2 text-center text-xs">
              ...
              <div class="rounded-md bg-card px-2 py-2">
                ...
                <div class="flex items-center justify-center gap-2">
                  <p class="font-mono text-sm font-semibold">
                    {{ xpPerMinute ? xpPerMinute.toLocaleString() : "—"
            ...
            <div v-if="partyXpProgress.length" class="space-y-1.5">
              ...
              >
                ...
                <div
                  class="relative h-1.5 flex-1 overflow-hidden rounded-full bg-muted/60"
                >
                  <div
                ...
                <span
                  class="w-7 shrink-0 font-mono text-[10px] font-semibold text-foreground"
                >
                  {{ entry.targetLevel }}
                </span>
              </div>
      ...
      >
        ...
        <div class="space-y-3 border-b border-border/70 px-4 py-3">
          ...
          <div class="flex flex-wrap gap-2 mt-2">
            <button
              class="pill focus-ring border-red-500/40 bg-red-500/10 text-red-500 hover:bg-red-500/20"
          ...
          <div class="flex items-start gap-2 rounded-lg bg-muted/30 px-3 py-2">
            ...
            <p class="text-[11px] text-muted-foreground">
              Level changes here are per-expedition only and do not update your
              collection.
            </p>
          </div>
        </div>
        ...
        <div
          class="max-h-[58vh] space-y-2 overflow-y-auto p-3 lg:min-h-0 lg:max-h-none lg:flex-1"
        >
          <button
            v-for="{
              creature,
            ...
            <div v-if="weightedStats.length" class="mt-3 space-y-1.5">
              ...
              >
                <span
                  ...
                  >{{ statAbbreviations[key] }}</span
                >
                <div class="h-1.5 rounded-full bg-muted">
                  <div
                    class="h-full rounded-full bg-primary"
                    :style="{
                      width: `${statBar(creature, key)}%`,
