...
<template>
  <section class="space-y-5 lg:space-y-6">
    ...
    <div class="surface-card p-4 sm:p-5">
      ...
      <div class="mt-4 space-y-3">
        ...
        <div class="flex flex-wrap items-center gap-2" role="radiogroup" aria-label="Filter by type">
          ...
          <button v-for="option in typeOptions" :key="option" role="radio" :aria-checked="typeFilter === option"
            class="pill focus-ring" :class="typeFilter === option ? 'pill-active' : ''" @click="typeFilter = option">
            <span v-if="option !== 'all'" class="mr-1.5 inline-block size-2 rounded-full"
        ...
        <div class="flex flex-wrap items-center gap-2" role="radiogroup" aria-label="Filter by tier">
          ...
          <button v-for="option in tierOptions" :key="option" role="radio" :aria-checked="tierFilter === option"
            class="pill focus-ring font-mono" :class="tierFilter === option ? 'pill-active' : ''"
            @click="tierFilter = option">
            {{ option === 'all' ? 'All' : `T${(option as number) + 1}` }}
          </button>
          <div class="ml-auto rounded-full border border-border bg-muted/40 px-3 py-1 text-xs font-semibold text-muted-foreground">
            {{ ownedCount }} / {{ totalCount }} summoned
          </div>
        </div>

        <!-- Bulk summon -->
        <div v-if="editing" class="flex flex-wrap items-center gap-2 border-t border-border/60 pt-3">
          ...
          <button class="pill focus-ring" @click="deselectAll">
    ...
    <div v-if="editing" class="flex items-start gap-3 rounded-xl border border-accent/25 bg-accent/5 px-4 py-3">
      <Info class="mt-0.5 size-4 shrink-0 text-accent" />
      <p class="text-sm text-muted-foreground">
        Click a creature card to mark it as summoned or not summoned. Set levels to match your in-game creatures.
        Your collection is saved locally and auto-fills levels in the expedition simulator.
      </p>
    </div>
    ...
    <div v-for="group in groupedByTier" :key="group.tier" class="space-y-3">
      ...
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
        <div v-for="creature in group.creatures" :key="creature.id"
          ...
          <!-- Type chips overlay -->
          <!-- Tier badge -->
          <span class="absolute -right-1.5 -top-1.5 z-10 rounded-md border border-border bg-card px-1.5 py-0.5 font-mono text-[10px] font-bold text-muted-foreground shadow-sm">
            T{{ creature.tier + 1 }}
          </span>

          <!-- Type chips -->
          <div class="absolute right-1.5 top-5 z-10 flex flex-col items-end gap-0.5">
            <span v-for="type in creature.types" :key="type" class="rounded-full px-1.5 py-px text-[10px] font-semibold leading-tight shadow-sm"
              :style="{
          ...
          <div class="flex items-center justify-center px-4 pb-5 pt-6"
            ...
            <img :src="getCreatureImage(creature)" :alt="creature.name" class="size-24 rounded-xl object-cover"
              loading="lazy" />
          </div>
    ...
    <div v-if="filteredCreatures.length === 0" class="surface-card px-4 py-8 text-center text-sm text-muted-foreground">
