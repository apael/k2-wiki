...
watch(() => route.query.creature, (val) => {
...
<template>
  <div class="space-y-6">
    <div class="space-y-2">
      <p class="text-[11px] font-bold uppercase tracking-[0.16em] text-muted-foreground">Level Up</p>
    ...
    <PlannerToolbar picker-label="Creature">
      ...
      <template #controls>
        <div class="flex items-center gap-2">
          ...
          <div class="inline-flex items-center overflow-hidden rounded-lg border border-border/70 bg-background/70">
            <button
              v-for="preset in [70, 120]"
              :key="preset"
              class="focus-ring h-8 px-3 text-sm font-semibold transition"
              :class="targetLevel === preset
