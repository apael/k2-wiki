...
const filtered = computed(() => {
  ...
  return sortedCreatures.value.filter(c =>
...
const selected = computed(() =>
  creatures.value.find(c => c.id === props.modelValue) ?? null,
)
  ...
  <div class="relative">
    ...
    <Teleport to="body">
      ...
      <div class="max-h-80 space-y-1 overflow-y-auto p-2">
        <p v-if="filtered.length === 0" class="px-3 py-4 text-center text-sm text-muted-foreground">
          No matches.
        </p>
        <button
          ...
          :class="creature.id === modelValue ? 'border-primary/45 bg-primary/10' : 'border-transparent hover:border-border/50 hover:bg-background/55'"
