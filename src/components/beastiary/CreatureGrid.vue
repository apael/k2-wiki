...
<template>
  <div class="grid">
    <CreatureCard v-for="creature in creatures" :key="creature.id" :creature="creature"
      :selected="selectedIds.includes(creature.id)" :selectable="true"
      @click="emit('creatureClick', creature)" @select="emit('toggleSelect', creature.id)" />
  </div>
...
.empty { text-align: center; padding: 48px 0; color: var(--color-text-muted); }
</style>
