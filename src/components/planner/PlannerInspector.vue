...
function getMethodTotalTime(method: PlannerMethod): number | null {
  ...
  if (
    props.activeMethod?.id === method.id &&
    props.schedule?.completionTimeByNode[node.id] != null
  ) {
...
const nodeSubtreeSummary = computed(() => {
  ...
  return {
    ...
    craftStepCount,
    branchPointCount,
    leafItems: [...leafAmounts.entries()].map(([itemId, { itemName, amount }]) => ({
      itemId,
      itemName,
      amount,
    })),
...
function openDisplayNodePlanner() {
  ...
  emit(
    'open-item-planner',
    displayNode.value.itemId,
    Math.max(1, Math.round(displayNode.value.requiredAmount)),
  )
}
</script>
  ...
  >
    ...
    >
      <div class="flex h-28 w-full items-center justify-center">
        <img
          v-if="getItemImage({ id: displayNode.itemId })"
        ...
        <span
          ...
          :style="{
            color: `color-mix(in oklch, ${itemTypeColor(displayNode.itemType)} 50%, transparent)`,
          }"
        >
      ...
      <h2 class="text-center text-xl font-black leading-tight text-foreground">
        {{ displayNode.itemName }}
      </h2>
      ...
      <div class="mt-2 flex flex-wrap justify-center gap-2">
        ...
        >
          ...
          <span class="font-mono" style="color: var(--color-primary)"
            >x{{ formatAmount(displayNode.requiredAmount) }}</span
          >
        </span>
      </div>
      ...
      <div v-if="!isRootNode" class="mt-3 flex flex-wrap justify-center gap-2">
        <button
          class="focus-ring bg-primary/12 hover:bg-primary/18 inline-flex items-center gap-2 rounded-full border border-primary/35 px-3 py-2 text-xs font-semibold text-primary transition"
      ...
      <p
        v-if="displayNode.issues.length"
      ...
      <p
        v-else-if="
          activeMethod && activeMethod.notes.length && activeMethod.localTimeSeconds == null
        "
        class="bg-yellow-400/8 mt-3 w-full rounded-lg border border-yellow-400/30 px-3 py-2 text-sm text-yellow-300"
      >
    ...
    <div
      v-else
      class="flex flex-col items-center justify-center px-5 py-16"
      :style="{
        background: `linear-gradient(180deg, color-mix(in oklch, var(--color-primary) 8%, transparent) 0%, transparent 100%)`,
      }"
    >
    ...
    <div class="px-5 pb-5">
      ...
      <section
        v-if="displayNode && methodOptions.length"
      ...
      >
        ...
        <div class="space-y-3">
          <div
            ...
            :key="method.id"
            class="overflow-hidden rounded-lg border border-l-[3px] pt-1 transition"
            :class="
          ...
          >
            ...
            <button
              class="focus-ring relative flex w-full items-center gap-2 px-3 py-3 pt-5 text-left transition"
            ...
            >
              ...
              <component
                :is="focusMethod?.id === method.id ? ChevronDown : ChevronRight"
              ...
              <div class="min-w-0 flex-1">
                ...
                <p
                  v-if="method.kind === 'gather' || method.kind === 'container'"
              ...
              <div class="shrink-0 text-right text-xs font-semibold">
                ...
                <template v-else-if="method.localTimeSeconds != null">
                  <span
                    ...
                    >{{ formatDuration(method.localTimeSeconds) }}</span
                  >
                  <template
                    v-if="
                      getMethodTotalTime(method) != null &&
                      getMethodTotalTime(method)! > method.localTimeSeconds
                    "
                  >
                    <span
                      class="font-mono text-muted-foreground"
                      title="Deps — time to complete all dependencies"
                      >/ +{{
                        formatDuration(getMethodTotalTime(method)! - method.localTimeSeconds)
                      }}</span
                    >
                    <span
                      class="font-mono text-foreground"
                      title="Total — step time plus dependencies"
                      >/ {{ formatDuration(getMethodTotalTime(method)!) }}</span
                    >
                  </template>
                </template>
              </div>
              <div
                class="hover:bg-primary/12 shrink-0 rounded-full p-1.5 transition hover:text-primary"
                :class="activeMethod?.id === method.id ? 'text-primary' : 'text-muted-foreground'"
                title="Pin this method"
                @click.stop="displayNode && emit('pin-method', displayNode.id, method.id)"
              >
                <Pin
                  class="size-3.5"
                  :fill="activeMethod?.id === method.id ? 'currentColor' : 'none'"
                />
              </div>
            </button>
            ...
            <div
              v-if="focusMethod?.id === method.id"
              class="space-y-3 border-t border-border/40 px-3 pb-3 pt-3"
            >
              <div class="grid grid-cols-2 gap-3">
                <div
                  v-for="detail in focusMethod.detailRows"
                  :key="`${focusMethod.id}-${detail.label}`"
                ...
                >
                  ...
                  >
                    {{ detail.estimated ? '≈ ' : '' }}{{ detail.value }}
                  </p>
                </div>
              </div>

              <div
                v-if="focusMethod.formula"
                class="rounded-lg border border-border/60 bg-background/45 px-4 py-3"
              >
              ...
              <div v-if="focusMethod.children.length" class="space-y-2">
                ...
                <button
                  v-for="child in focusMethod.children"
                  :key="child.nodeId"
                ...
                >
                  <div class="flex items-center gap-2">
                    <img
                      v-if="getItemImage({ id: child.itemId })"
                    ...
                    <div>
                      <p class="text-sm font-semibold text-foreground">
                        {{ nodesById[child.nodeId]?.itemName ?? child.itemId }}
                      </p>
                      <p class="mt-1 text-xs text-muted-foreground">
                        Need
                        <span class="font-mono" style="color: var(--color-primary)"
                          >x{{ formatAmount(child.amount) }}</span
                        >
                      </p>
                    </div>
                  </div>
      ...
      <!-- Subtree Totals -->
      <section
        v-if="nodeSubtreeSummary && nodeLeafMaterials.length > 1"
        class="mt-4 border-t border-border/60 pt-4"
      >
        <h3 class="mb-3 text-xs font-bold uppercase tracking-[0.16em] text-muted-foreground">
          Subtree Totals
        ...
        <div class="grid grid-cols-2 gap-3">
          <div
            class="rounded-lg border border-border/60 bg-background/45 px-3 py-3"
            title="Time for this action alone"
          >
            <p class="text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">
              Step Time
            </p>
            <p class="mt-1 font-mono text-sm font-semibold" style="color: var(--color-green)">
              {{
                nodeSubtreeSummary.stepTimeSeconds != null
                  ? formatDuration(nodeSubtreeSummary.stepTimeSeconds)
                  : 'N/A'
              }}
            </p>
          </div>
          <div
            class="rounded-lg border border-border/60 bg-background/45 px-3 py-3"
            title="Time to complete all dependencies (parallel across resources)"
          >
            <p class="text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">
              Deps Time
            </p>
            <p class="mt-1 font-mono text-sm font-semibold" style="color: var(--color-green)">
              <template
                v-if="
                  nodeSubtreeSummary.depsTimeSeconds != null &&
                  nodeSubtreeSummary.depsTimeSeconds > 0
                "
                >{{ formatDuration(nodeSubtreeSummary.depsTimeSeconds) }}</template
              >
          ...
          <div
            class="rounded-lg border border-border/60 bg-background/45 px-3 py-3"
            title="Step time plus dependencies — total time to produce this item"
          >
            <p class="text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">
              Total Time
            </p>
            <p class="mt-1 font-mono text-sm font-semibold" style="color: var(--color-green)">
              {{
                nodeSubtreeSummary.totalTimeSeconds != null
                  ? formatDuration(nodeSubtreeSummary.totalTimeSeconds)
                  : 'N/A'
              }}
            </p>
          </div>
        ...
        <p class="mt-3 flex items-start gap-2 text-sm leading-relaxed text-muted-foreground">
          ...
          <span
            >Times assume independent steps run in parallel (e.g. gathering different materials
            simultaneously) while dependent steps run in series (e.g. gathering before crafting).
            Total time reflects the longest parallel path, not the sum of all steps.</span
          >
        </p>
      </section>

      <!-- Leaf Materials (node-scoped) -->
      <section v-if="nodeLeafMaterials.length > 1" class="mt-4 border-t border-border/60 pt-4">
        <h3 class="mb-3 text-xs font-bold uppercase tracking-[0.16em] text-muted-foreground">
          Leaf Materials
        </h3>
        <div class="space-y-1">
          <div
            v-for="leaf in nodeLeafMaterials"
            :key="leaf.itemId"
